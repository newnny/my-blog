...
const Works: React.FC = () => {
    ...
    const handleClickCategory = (id: number, name: string) => {
        setList(
            list.map(item => {
                if (item.id === id) {
                    return { ...item, textColor: "#000000", select: true, fontWeight: 700 }
                }
                return { ...item, textColor: "#828282", select: false, fontWeight: 500 }
            })
        )
            ...
            {list.map(i => (
                <div className="flex-row" style={{ marginTop: 10, marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", width: "100%" }}>
                        <Grid container direction={matches ? "column" : "row"} style={{ minHeight: 90 }}>
                            <Grid item sm={2} xs={12} style={{ display: "flex", alignItems: "center" }}>
                                <Typography
                                    key={i.id}
                                    style={{ color: i.textColor, cursor: "pointer", fontSize: 16, padding: 10, fontWeight: i.fontWeight }}
                            ...
                            {selectedItem !== "" &&i.select &&
                                <Grid item sm={10} xs={12}>
                                    {selectedItem === "RAMP medical" && i.select &&
                                        <div className="flex-column" style={{ padding: 10 }}>
                                            <div className="flex-center">
                                                <img style={{ width: matches ? "100%" : "30%" }} src={ramp} alt="ramp" />
                                            </div>
                                            <div className="flex-center">
                                                <p>
                                    ...
                                    {selectedItem === "Miniflux e-pub binder" && i.select &&
                                        <div className="flex-column" style={{padding: 10}}>
                                            <div className="flex-center">
                                                <img style={{ width: matches ? "100%" : "30%" }} src={minifluxbinder} alt="minifluxbinder" />
                                            </div>
                                            <div className="flex-center">
                                                <p>
                                    ...
                                    {selectedItem === "Working with D3.js" && i.select &&
                                        <a className="a" href="https://visualisations-one.vercel.app/" target="_blank" rel="noreferrer" style={{ padding: 10 }}>
                                            view D3 project
                                        </a>
                                    }
                                </Grid>}
